...
# Function to remove unwanted words from the text
...
async def pm_search(client, message):
    ...
    if ispm:
        ...
        # If PM mode is not enabled, reply with a message
        await message.reply_text(
            text=f"<b>𝗛𝗲𝗹𝗹𝗼 𝗱𝗲𝗮𝗿 {message.from_user.mention},\n\n𝘐 𝑐𝑎𝑛'𝑡 𝑔𝑖𝑣𝑒 𝑚𝑜𝑣𝑖𝑒𝑠 𝑡𝑜 𝑦𝑜𝑢 𝒉𝑒𝑟𝑒.\n𝘐 𝑤𝑜𝑟𝑘 𝑜𝑛𝑙𝑦 𝑖𝑛 𝑔𝑟𝑜𝑢𝑝𝑠.</b>",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("🪄 𝐑𝐞𝐪𝐮𝐞𝐬𝐭 𝐇𝐞𝐫𝐞 🌝", url="https://t.me/+55RV97igBkQ4ZmVl"
                        )
                    ]
                ]
            ),
...
async def group_search(client, message):
    ...
    if not user_id:
        return await message.reply("<b>🚨 ɪ'ᴍ ɴᴏᴛ ᴡᴏʀᴋɪɴɢ ғᴏʀ ᴀɴᴏɴʏᴍᴏᴜꜱ ᴀᴅᴍɪɴ!</b>")

    if text.startswith("/"):
        return

    # Force Subscribe check
    if not await is_user_fsub(client, message):
        return

    settings = await get_settings(chat_id)

    # 🟡 SUPPORT GROUP logic (special case)
    if chat_id == SUPPORT_GROUP:
        files, offset, total = await get_search_results(text, offset=0)
        if total != 0:
            return await send_files(client, message, files)

        # fallback fuzzy match
        from fuzzywuzzy import process
        search_list = await get_all_filenames()  # You must define this
        best_match = process.extractOne(text, search_list)

        if best_match:
            fuzzy_files, _, fuzzy_total = await get_search_results(best_match[0], offset=0)
            if fuzzy_total != 0:
                return await send_files(client, message, fuzzy_files)
    ...
    # 🔗 Anti-link (optional)
    if re.findall(r'https?://\S+|www\.\S+|t\.me/\S+', text):
        if not await is_check_admin(client, chat_id, user_id):
            await message.delete()
            return await message.reply("<b>🚫 sᴇɴᴅɪɴɢ ʟɪɴᴋs ɪs ɴᴏᴛ ᴀʟʟᴏᴡᴇᴅ ʜᴇʀᴇ ❌</b>")

    # ⚠️ Admin mentions (optional)
    if "@admin" in text.lower() or "@admins" in text.lower():
        if not await is_check_admin(client, chat_id, user_id):
            admins = [m.user.id async for m in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS) if not m.user.is_bot]
            try:
    ...
    if settings.get("auto_filter", False):
        ...
        msg = await message.reply_text("<b>⚠️ ᴀᴜᴛᴏ ꜰɪʟᴛᴇʀ ᴍᴏᴅᴇ ɪꜱ ᴏꜰꜰ...</b>")
...
async def year_search(client: Client, query: CallbackQuery):
    ...
    btn.insert(2,[
        InlineKeyboardButton("🌚 𝐒𝐞𝐧𝐝 𝐀𝐥𝐥 🌝", callback_data=batch_link),
        ])
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        [
            ...
            InlineKeyboardButton('✨𝐔𝐩𝐝𝐚𝐭𝐞𝐬 🔔', url=f'https://t.me/+sQXky-6HHq8xMTk1')
        ],
        [
            InlineKeyboardButton('💎 𝐏𝐫𝐞𝐦𝐢𝐮𝐦 𝐌𝐨𝐯𝐢𝐞𝐬 🔐', url=f'https://t.me/+02e7v00GQ4o2MDA1')
        ]]    
